          ...
          <ol className="divide-y divide-board bg-board-deep/40 border border-board rounded-lg overflow-hidden">
            {scores.map((row, i) => (
              <li key={`${row.name}-${i}`} className="flex items-center gap-3 px-4 py-2.5">
                <span className="text-blue-200/60 w-8 text-right tabular-nums">{i + 1}.</span>
                <span className="flex-1 truncate">{row.name}</span>
                <span className="text-blue-200/60 text-sm hidden sm:inline">
                  {row.correct}✓ {row.wrong}✗ {row.passed}–
                </span>
                <span className="text-blue-200/60 text-sm tabular-nums w-12 text-right">
