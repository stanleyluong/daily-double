...
function StatTile({ label, value }: { label: string; value: string }) {
  return (
    <div className="bg-board-deep/50 border border-board rounded-lg px-3 py-3 text-center">
      ...
      <p className="text-[11px] uppercase tracking-wider text-blue-200/60 mt-0.5">{label}</p>
    </div>
  );
}
...
function MyScoresPageInner() {
  ...
  // Filter/sort state initializes from (and writes back to) the URL, so a
  // filtered view survives refresh and can be shared as a link.
  const urlType = searchParams.get("type");
  const urlProgress = searchParams.get("progress");
  const urlSort = searchParams.get("sort");
  const urlDir = searchParams.get("dir");
  const initialSortKey: SortKey = SORT_COLUMNS.some((c) => c.key === urlSort) ? (urlSort as SortKey) : "lastPlayed";
  const [sortKey, setSortKey] = useState<SortKey>(initialSortKey);
  const [sortDesc, setSortDesc] = useState(
    urlDir === "asc" || urlDir === "desc" ? urlDir === "desc" : (DEFAULT_DESC[initialSortKey] ?? false)
  );
  const [typeFilter, setTypeFilter] = useState<TypeFilter>(
    urlType === "daily" || urlType === "historical" || urlType === "custom" ? urlType : "all"
  );
  const [progressFilter, setProgressFilter] = useState<ProgressFilter>(
    urlProgress === "in_progress" || urlProgress === "completed" ? urlProgress : "all"
  );
  const [query, setQuery] = useState(searchParams.get("q") ?? "");

  // Replace, not push — the back button shouldn't step through filter clicks.
  useEffect(() => {
    const params = new URLSearchParams();
    if (typeFilter !== "all") params.set("type", typeFilter);
    if (progressFilter !== "all") params.set("progress", progressFilter);
    if (query.trim()) params.set("q", query.trim());
    if (sortKey !== "lastPlayed") params.set("sort", sortKey);
    if (sortDesc !== (DEFAULT_DESC[sortKey] ?? false)) params.set("dir", sortDesc ? "desc" : "asc");
    const qs = params.toString();
    router.replace(`/history${qs ? `?${qs}` : ""}`, { scroll: false });
  }, [typeFilter, progressFilter, query, sortKey, sortDesc, router]);
    ...
    <div className="flex flex-col flex-1 min-h-screen">
      <main className="flex-1 w-full max-w-5xl mx-auto px-4 md:px-8 py-10">
        ...
        {user && streak && streak.current > 0 && (
          <p className="text-center mb-4">
            ...
            {streak.longest > streak.current && (
              <span className="text-blue-200/60 text-sm ml-2">(best: {streak.longest})</span>
            )}
          </p>
        )}
        ...
        {user && history && history.length > 0 && (
          <div className="flex flex-col gap-3 mb-4">
            <div className="flex flex-wrap items-center justify-center gap-2">
              ...
              <span className="text-blue-200/60 mx-1">·</span>
            ...
            <input
              ...
              className="w-full max-w-sm mx-auto rounded-lg bg-board border border-blue-300/30 focus:border-gold outline-none px-4 py-2 text-sm placeholder:text-blue-200/60"
            />
          </div>
        )}
        ...
        {loading ? (
          ...
          <p className="text-center text-blue-200/60 py-16">No games match your filters.</p>
        ) : (
                ...
                {visibleHistory!.map((row) => {
                  ...
                  return (
                    <tr key={row.id} className="border-t border-board hover:bg-board-deep/40">
                      ...
                      <td className="px-4 py-3 align-top text-blue-200/60 whitespace-nowrap">
